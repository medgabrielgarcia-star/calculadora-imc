...
const resultados = document.querySelector(".calc-imc__resultados");
const rangoNormal = document.querySelector(".calc-imc__rango-items.normal");
const rangoSobrepeso = document.querySelector(".calc-imc__rango-items.sobrepeso");
const rangoObesidad = document.querySelector(".calc-imc__rango-items.obesidad");


function limpiarRangos() {
    rangoNormal.classList.remove("activo-normal");
    rangoSobrepeso.classList.remove("activo-sobrepeso");
    rangoObesidad.classList.remove("activo-obesidad");
}
...
formulario.addEventListener("submit", function(event) {
    ...
    if (pesoUser > 0 && alturaUser > 0) {
        ...
        resultados.innerHTML = `
            Tu IMC es: <center><b>${imc.toFixed(2)}</b></center> <br>
            Tu clasificación es: <center><b>${infoImc.texto}</b></center>`;
        resultados.style.color = infoImc.clase;

        // --- Lógica de Confetis y Burbuja ---

        if (imc < 18.5) {

            textoBurbuja.innerHTML = `
                Tu IMC es algo bajo.
                Un peso más saludable para ti sería de
                <b>${datosPesoIdeal.ideal} kg</b>.
            `;
            ...
            textoBurbuja.innerHTML = `
                ...
                Mantente entre los
                <b>${datosPesoIdeal.min}</b>
                y
                <b>${datosPesoIdeal.max} kg</b>.
            `;

            rangoNormal.classList.add("activo-normal");

        } else if (imc < 29.9) {


            textoBurbuja.innerHTML = `
                Un peso ideal sería acercarse a los
                <b>${datosPesoIdeal.ideal} kg</b>.
            `;
            rangoSobrepeso.classList.add("activo-sobrepeso");

        } else {

            textoBurbuja.innerHTML = `
                Un peso saludable para tu altura es cerca de
                <b>${datosPesoIdeal.ideal} kg</b>.
            `;
            rangoObesidad.classList.add("activo-obesidad");
        }
        ...
        resultado.textContent =
            "Por favor, ingresa datos correctos.";
        textoBurbuja.textContent =
            "Por favor, ingresa valores válidos para poder ayudarte.";
    }
});
...
if (botonLimpiar) {
    ...
    botonLimpiar.addEventListener("click", function() {
        ...
        resultado.innerHTML = "";
        textoBurbuja.textContent =
            "Introduce tus datos para que pueda darte una recomendación.";
...
function clasificarimc(imc) {
    if (imc < 18.5)
        return {
            texto: "Bajo peso",
            clase: "var(--color-imc-low)"
        };
    if (imc < 24.9)
        return {
            texto: "Peso saludable",
            clase: "var(--color-imc-normal)"
        };
    if (imc < 29.9)
        return {
            texto: "Sobrepeso",
            clase: "var(--color-imc-overweight)"
        };
    if (imc < 34.9)
        return {
            texto: "Obesidad I",
            clase: "var(--color-imc-obese-i)"
        };
    if (imc < 39.9)
        return {
            texto: "Obesidad II",
            clase: "var(--color-imc-obese-ii)"
        };
    return {
        texto: "Obesidad III (Grave)",
        clase: "var(--color-imc-obese-iii)"
    };
}
